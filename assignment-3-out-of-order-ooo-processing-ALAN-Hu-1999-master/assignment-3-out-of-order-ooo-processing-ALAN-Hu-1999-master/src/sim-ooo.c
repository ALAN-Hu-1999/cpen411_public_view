...
// generated hit/miss rate and emulate the latency of cahce architechture 
int rand_load_lat()
{
    int lat = 1;
...
static void cpen411_init()
{
    ...
    // empty IFQ
    g_ifq_head = g_ifq_tail = g_ifq_count = 0;  // instuction fetch queue
    g_rob_head = g_rob_tail = g_rob_count = 0;  // rob
...
static void fetch(void)
{
    ...
    if (g_ifq_count < IFQ_SIZE) {
        ...
        g_fetch_pc += sizeof(md_inst_t);  // increment to next pc 
    }
}

// simliar to a decode state 
static void dispatch(void)
{
    if ((g_ifq_count > 0) && (g_rob_count == 0)) {  // when instruction is loaded and the rob is empty
        inst_t *pI = g_ifq[g_ifq_head];
        ...
        g_rob[g_rob_tail] = pI; // store it at the bottom of rob
        g_rob_tail = (g_rob_tail + 1) % ROB_SIZE; // keep the index of rob <= 32
        g_rob_count++;  // adding inst. to rob
        g_ifq_head = (g_ifq_head + 1) % IFQ_SIZE; // keep the index of rob <= 8
        g_ifq_count--;  // removing queunig inst from ifq
...
#include "machine.def"
	...
	pI->next_pc = regs.regs_PC;
        ...
        //fprintf(stderr, "uid = %d, i2 = %d, i3 = %d, o1 = %d, o2 = %d \n", pI->uid, i2, i3, o1, o2);
        int i; int j;
        for(i = 2; i > 0; i--){
          for(j = 0; j<5;j++){
            dependency_track[i][j] = dependency_track[i-1][j];
          }
        }
        
        // update the latest src & dst register
        dependency_track[0][0] = i1;
        dependency_track[0][1] = i2;
        dependency_track[0][2] = i3;
        dependency_track[0][3] = o1;
        dependency_track[0][4] = o1;
...
static void issue_and_execute()
{
    ...
    for (i = 0; i < g_rob_count; ++i) {
        ...
        if (pI->status == DISPATCHED) { // not yet executed
            ...
            // potentially solution: check pI->next
            //fprintf(stderr, "rob = %d rob head = %d rob tail = %d\n", g_rob_count, g_rob_head, g_rob_tail);
            // dynamic dependency
            int i;  int j;  int k;

            // for(k = 0; k<3; k++){
            //   if(dependency_track[0][k] != 0){
            //     for (i = 1; i < 3; i++){
            //       for(j = 0; j < 5; j++){
            //         if(dependency_track[0][k] == dependency_track[i][j]){
            //           fprintf(stderr, "%d %d\n", dependency_track[0][k], dependency_track[i][j]);
            //           break;
            //         } 
            //       }
            //     }
...
void retire(void)
{
    if (g_rob_count > 0) {
        ...
        g_rob_head = (g_rob_head + 1) % ROB_SIZE; // increament rob head
        g_rob_count--;
